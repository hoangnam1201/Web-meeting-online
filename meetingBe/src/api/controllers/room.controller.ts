    ...
    try {
      if (!req.query.pageIndex && !req.query.pageSize) {
        const rooms = await roomModel
          .find({ owner: new mongoose.Types.ObjectId(userId) as any })
          .populate("owner");
      ...
      const roomData = await roomModel
        ...
        .limit(pageSize);
      const roomCount = await roomModel
        .find({ owner: new mongoose.Types.ObjectId(userId) as any })
        .countDocuments();
      const roomDataReads = RoomReadDto.fromArray(roomData);
      const pageData = PageDataService.getPageData(
        roomDataReads,
        roomCount,
        pageIndex,
        pageSize
  ...
  addMember = async (req: Request, res: Response) => {
    ...
    try {
      ...
      await userModel.updateOne(
        { _id: userId },
        { $addToSet: { invitedRooms: new mongoose.Types.ObjectId(roomId) as any } }
      );
      res.status(200).json({ status: 200, data: null });

      const msg = `You are invited to class ${room.name}`;
      const notificationCreate = NotificationCreateDto.fromUserNotification(
        msg,
        authId,
        userId
      );
